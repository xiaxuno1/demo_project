...
def file_copy2(src_path,dst_path):
    ...
    :return:
    """
    if os.path.isdir(dst_path): #对传入dst为路径的适用
        dst_path = os.path.join(dst_path, os.path.basename(src_path))
    if os.path.isfile(dst_path):  #文件存在
        if check_file_isupdate(src_path,dst_path): #先检查文件是否存在再检查文件有修改时执行复制
            shutil.copy2(src_path,dst_path)
    elif not os.path.isfile(dst_path):  #文件不存在
        shutil.copy2(src_path, dst_path)
